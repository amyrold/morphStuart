...
# PALEO-ECOLOGICAL DATA PROCESSING
## Data Import
```{r}
...
```{r}
# Extract and standardize sample metadata
...
processing_log <- log_step(
  ...
  notes = "Extracted V-numbers, bins, and sample types for data linking"
)
...
metadata_summary <- paleo_metadata %>%
  summarise(
    ...
    samples_with_bins = sum(!is.na(bin)),
...
```{r}
# Handle multiple microscopy line counts per sample (Jacopo's requirement)
...
processing_log <- log_step(
  ...
  notes = "Merged multiple microscopy line counts per geological sample"
)
```

## Standardize Taxonomy
```{r}
# Apply Jacopo's taxonomic preferences (morphotype-level for diatoms)
standardized_taxonomy <- standardize_taxonomy(merged_diatom_counts, microfossil_type = "Diatom")
...
processing_log <- log_step(
  ...
  output_data = standardized_taxonomy$data,
  notes = "Standardized to morphotype-level taxonomy per Jacopo's specifications"
)

# Export taxa list for Jacopo's ecological review
write.csv(standardized_taxonomy$taxonomy_summary, 
...
processing_log <- log_step(
  ...
  input_data = standardized_taxonomy$data,
...
## Validate Processing
```{r}
# Comprehensive validation matching morphology pipeline style
paleo_validation <- validate_paleo_data(paleo, rioja_diatoms, 
                                       list(metadata = paleo_metadata,
                                            merged_counts = merged_diatom_counts,
                                            taxonomy = standardized_taxonomy,
                                            ages = paleo_with_ages))

# ADD VALIDATION (FIXED CUSTOM CHECKS):
validate_step(
  rioja_diatoms$counts, 
  step_name = "Paleo Matrix Validation",
  custom_checks = list(
    matrix_completeness = function(data) {
      list(
        passed = all(rowSums(data) > 0),
        message = "All samples have at least one taxon count"
      )
    },
    sample_ordering = function(data) {
      samples_with_strat <- rioja_diatoms$samples %>% filter(!is.na(strat_level))
      list(
        passed = nrow(samples_with_strat) > 0,
        message = paste("Stratigraphic ordering available for", nrow(samples_with_strat), "samples")
      )
    }
  )
)
...
# If the above still causes issues, try this more conservative approach
if(rioja_diatoms$has_age_data) {
  ...
  # Simple stratigraphic plot without clustering first
  svg(paste0(p$`3.outputs`, "simple_diatom_diagram.svg"), 
      width = 12, height = 8)
  
  # Minimal parameters to avoid conflicts
  strat.plot(
    plot_matrix[, 1:min(11, ncol(plot_matrix))],  # Limit to 8 taxa
    yvar = plot_depths,
    y.rev = TRUE,
    scale.percent = TRUE
  )
  ...
  # Add clustering in separate step if the simple plot works
  if(nrow(plot_matrix) > 3) {
    diss <- dist(sqrt(plot_matrix/rowSums(plot_matrix)))
    clust <- chclust(diss, method = "coniss")
    
    svg(paste0(p$`3.outputs`, "diatom_with_clustering.svg"), 
        width = 14, height = 10)
  ...
  cat("Alternative plots created successfully!\n")
}
```

## Export Results
```{r}
# Export in format requested by Jacopo
...
processing_log <- log_step(
  ...
  input_data = rioja_diatoms$counts,  # Add this required parameter
  files_created = exported_files,
  notes = "Exported summary tables and taxa lists for ecological grouping"
)

cat("Paleo-ecological data processing complete!\n")
cat("Files ready for Jacopo's ecological grouping review.\n")
```
...
paleo_quality <- create_quality_report(rioja_diatoms$counts, 
                                       output_dir = paste0(p$`4.docs`, "reports"))
...
ggsave(paste0(p$`3.outputs`, "paleo_sample_distribution.svg"), p_sample_dist, width = 8, height = 6)
ggsave(paste0(p$`3.outputs`, "top_taxa_abundance.svg"), p_taxa_abundance, width = 10, height = 8)
```
