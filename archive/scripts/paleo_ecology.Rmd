...
#out <- lapply(paleo[c("Morphotype", "Genus_Type", "Species")], function(x) sort(unique(x[!is.na(x) & x != ""])))
...
# Access matrices
stickleback_matrix <- results$stickleback$counts  # rownames = LSPEC
killifish_matrix <- results$killifish$counts      # rownames = V_number
summary_stats <- results$morphotype_summary       # combined analysis

# This gives you both diatom and phytolith morphotypes in one analysis-ready matrix!
