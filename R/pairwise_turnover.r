...
calculate_pairwise_turnover <- function(data, method = "bray", binary = FALSE) {
  ...
  community_matrix <- data %>%
    select(CSTRAT, all_of(taxa_cols)) %>%
    arrange(CSTRAT)
  ...
  if (nrow(cstrat_counts) > 0) {
    ...
    cat("Pooling", sum(cstrat_counts$n), "samples with duplicate CSTRAT values for turnover analysis\n")
  ...
  rownames(species_matrix) <- paste0("CSTRAT_", community_matrix$CSTRAT)
  ...
  rownames(formatted_matrix) <- community_matrix$CSTRAT
  colnames(formatted_matrix) <- community_matrix$CSTRAT
