...
filter_rare_taxa <- function(data, threshold = 0.10) {
  ...
  occurrence_rates <- data_df %>%
    ...
    tidyr::pivot_longer(everything(), names_to = "taxon", values_to = "occurrences") %>%
    dplyr::mutate(
      occurrence_rate = occurrences / n_samples,
      meets_threshold = occurrence_rate >= threshold
    ) %>%
    dplyr::arrange(dplyr::desc(occurrence_rate))
  ...
  filtered_data_df <- data_df %>%
    dplyr::select(dplyr::all_of(c(available_metadata, taxa_to_keep)))
  ...
  if ("total_count" %in% available_metadata && length(taxa_to_keep) > 0) {
    filtered_data_df <- filtered_data_df %>%
      dplyr::mutate(total_count = rowSums(dplyr::select(., dplyr::all_of(taxa_to_keep))))
  }
  
  if ("n_taxa" %in% available_metadata && length(taxa_to_keep) > 0) {
    filtered_data_df <- filtered_data_df %>%
      dplyr::mutate(n_taxa = rowSums(dplyr::select(., dplyr::all_of(taxa_to_keep)) > 0))
  }
  ...
  if (is_matrix_input) {
    ...
    if ("LSPEC_rownames_temp" %in% names(filtered_data_df)) {
      ...
      filtered_data_df <- dplyr::select(filtered_data_df, -LSPEC_rownames_temp)
    }
...
get_taxa_occurrence_summary <- function(data, threshold = 0.10) {
  ...
  occurrence_summary <- data %>%
    ...
    tidyr::pivot_longer(everything(), names_to = "taxon_stat", values_to = "value") %>%
    tidyr::separate(taxon_stat, into = c("taxon", "statistic"), sep = "_(?=[^_]*$)") %>%
    tidyr::pivot_wider(names_from = statistic, values_from = value) %>%
    dplyr::mutate(
      occurrence_rate = occurrences / n_samples,
      meets_threshold = occurrence_rate >= threshold,
      rarity_category = dplyr::case_when(
