...
merge_non_overlap <- function(data) {
  ...
  for (i in seq_along(fish_ids)) {
    ...
    # If only one part type exists, use that directly
    if (nrow(part_data) == 0 || nrow(cpart_data) == 0) {
      result_row <- if(nrow(part_data) > 0) part_data[1,] else cpart_data[1,]
    ...
    # Handle metadata - use first part row for metadata
    ...
    # For each measurement column, merge values prioritizing part over counterpart
    for (col in measure_cols) {
      ...
      # Get values from part and counterpart
      ...
      if (length(part_vals) > 0) {
        part_val <- if (var_type %in% c("binary", "count")) max(part_vals) else mean(part_vals)
      }
      
      if (length(cpart_vals) > 0) {
        cpart_val <- if (var_type %in% c("binary", "count")) max(cpart_vals) else mean(cpart_vals)
      }
