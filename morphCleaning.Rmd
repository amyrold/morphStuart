...
missing_parts <- fish_part_counts %>%
  filter(P == 0 | C == 0)
sum(missing_parts$`NA`) # fish without both "part" & "counter part"


# Fish with excessive rows
multi_row_fish <- fish_part_counts %>%
  filter(P > 1 | C > 1)
length(multi_row_fish$fish_id) # fish with duplicate "parts" or "counter parts"
```
...
# Summary table of specimens by bin with age data
bin_summary <- morph_with_age %>%
  group_by(bin) %>%
  summarise(
    n_specimens = n(),
    has_age_data = any(!is.na(CSTRAT)),
    mean_cstrat = mean(CSTRAT, na.rm = TRUE),
    mean_year = mean(YEAR, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(desc(n_specimens))

# Show top bins by specimen count
cat("\nTop 10 bins by specimen count:\n")
print(head(bin_summary, 10))
```

## Handle Flagged Fish with Age Data
```{r Flagged Fish Age Data}
# If you also want to add age data to the flagged fish for review
# Read the flagged fish data
flagged_fish <- overlap_results$overlap_fish

# Add age data to flagged fish as well
flagged_with_age <- flagged_fish %>%
  left_join(age_formatted, by = "bin")

# Save flagged fish with age data
write.csv(flagged_with_age,
          file = paste0(p$`1.data/c.flagged`, "/flagged_fish_with_age.csv"),
          row.names = FALSE)

cat("\nFlagged fish with age data saved to:", 
    paste0(p$`1.data/c.flagged`, "/flagged_fish_with_age.csv"))
```
...
write.csv(morph_with_pgna, file = paste0(p$`1.data/a.raw`, "020625_PitLMorph.csv"))
